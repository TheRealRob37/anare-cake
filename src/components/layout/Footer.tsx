...
export default function Footer() {
  ...
  const LINKS = {
    [t.footer.explore]: [
      { href: "/",             label: t.footer.link_home    },
      { href: "/menu",         label: t.footer.link_cakes   },
      { href: "/configurator", label: t.footer.link_design  },
      { href: "/about",        label: t.footer.link_story   },
    ],
    [t.footer.order]: [
      { href: "/contact",   label: t.footer.link_contact   },
      { href: "/faq",       label: t.footer.link_faq       },
      { href: "/delivery",  label: t.footer.link_delivery  },
      { href: "/weddings",  label: t.footer.link_weddings  },
    ],
  };
    ...
    <footer className="bg-ink-900 text-cream-200 pt-16 pb-8">
      <div className="container-site">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 pb-12 border-b border-ink-700">
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            <a
              ...
              className="inline-flex items-center gap-2 text-xs text-gold-300 hover:text-gold-200 transition-colors"
            >
          ...
          {Object.entries(LINKS).map(([title, links]) => (
            <div key={title} className="space-y-4">
              ...
              <ul className="space-y-2">
                {links.map((l) => (
                  <li key={l.href}>
                    <Link href={l.href} className="text-sm text-ink-300 hover:text-cream-100 transition-colors duration-200">
                      {l.label}
                    </Link>
                  </li>
                ))}
        ...
        <div className="pt-8 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs text-ink-500">
          <p>© {new Date().getFullYear()} Anare Cake. {t.footer.rights}</p>
